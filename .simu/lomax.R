...
random_number <- sample.int(1e6,1)
set.seed(98323 + random_number) # random hand typing algorithm
...
for(m in na.omit(ind[id_slurm,])){
  ...
  if(!is.null(fit_sample)){
    ...
    if(!is.null(ci_class)) res$classic[m,] <- ci_class
    t2 <- Sys.time()
    res$time[m,7] <- difftime(t2, t1, units = "secs")
  }
